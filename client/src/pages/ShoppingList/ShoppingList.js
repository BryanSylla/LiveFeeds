...
class ShoppingList extends Component {
  ...
  componentDidMount() {
    ...
    this.loadShoppingList(this.state.sessionusername);
     API.getemail(this.state.sessionusername)
...
sendEmail =(shoppinglist) =>{
 

  API.sendemail(this.state.email,shoppinglist)
      ...
      <Container fluid>
         <Row>
          ...
          <Navlo />
            ...
            (<button onClick={() => this.sendEmail(this.state.ShoppingList)}>Send Shopping List to Email</button>):(<h5>There is no Shopping List to Email</h5>) }
            {this.state.ShoppingList.length ? (
              <List>
